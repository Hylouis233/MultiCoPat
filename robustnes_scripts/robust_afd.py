...
AFD_STL_CONFIG = DEFAULT_AFD_STL_CONFIG
...
def load_stl_output(stl_output_dir=None):
    ...
    raise FileNotFoundError(f"STL arrays not found in {arrays_dir}")
...
def process_single_afd_series_adaptive(series_info, component_type='seasonal', linear_trend_adjust=None):
    ...
    if AFD_STL_CONFIG.get('skip_afd', False):
        result = {
            'optimal_K': 0,
            'decomposed_components_processed': [],
            'residue_component_processed': series_data,
            'processed_signal': series_data,
            'linear_trend': np.zeros_like(series_data),
            'mean': np.mean(series_data),
            'std': np.std(series_data),
            'original_data': series_data,
            'normalized_data': series_data,
            'p_value_history': [],
    ...
    if 'AFDCal' not in sys.modules:
        sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../scripts/AFDCal')))
    ...
    for K in range(1, max_levels + 1):
        ...
        if hasattr(afdcal, 'deComp') and len(afdcal.deComp) > 0:
...
def perform_afd_stl_analysis_parallel(stl_series_dict, component_type='seasonal'):
    ...
    if AFD_STL_CONFIG.get('skip_afd', False):
        print(">> SKIPPING AFD DECOMPOSITION (Ablation Study Mode) <<")
    print("=" * 80)
...
def save_afd_stl_results(seasonal_results, residual_results, time_index, series_names, output_dir=None):
    if output_dir is None:
        output_dir = AFD_STL_CONFIG['output_dir']
        
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    
    print("\\n" + "=" * 80)
    print("Saving AFD-STL Analysis Results")
    print("=" * 80)
    ...
    elif max_K_R == 0 and n_timepoints > 0:
        residual_3d_array = np.full((n_series, n_timepoints, 1), np.nan)
        for i, series_name in enumerate(series_names):
            if residual_results and series_name in residual_results:
                result = residual_results[series_name]
                residue = result.get('residue_component_processed', None)
                if residue is not None and len(residue) == n_timepoints:
                    residual_3d_array[i, :, 0] = residue
        residual_npy = output_path / 'afd_residual_components.npy'
        np.save(residual_npy, residual_3d_array)
...
def run_afd(config=None, output_dir=None):
    global AFD_STL_CONFIG, PARALLEL_CONFIG
    
    if config:
        if 'afd_config' in config:
            AFD_STL_CONFIG.update(config['afd_config'])
        if 'parallel_config' in config:
            PARALLEL_CONFIG.update(config['parallel_config'])
            
    if output_dir:
        AFD_STL_CONFIG['output_dir'] = str(output_dir)
    
    print("=" * 80)
    print("AFD-STL Analysis Module (Robustness Run)")
    print("=" * 80)
    ...
    print(f"  Output directory: {AFD_STL_CONFIG['output_dir']}")
    print(f"  Skip AFD: {AFD_STL_CONFIG.get('skip_afd', False)}")
    print("=" * 80)
    
    stl_series_dict, time_index, stl_metadata = load_stl_output(AFD_STL_CONFIG['stl_output_dir'])
