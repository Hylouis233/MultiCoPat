...
def run_stl(config: Optional[Dict] = None, 
           output_dir: Optional[Union[str, Path]] = None):
    global STL_CONFIG, PARALLEL_CONFIG, VISUALIZATION_CONFIG, OUTPUT_CONFIG
    
    if config:
        if 'stl_config' in config:
            STL_CONFIG.update(config['stl_config'])
    ...
    print("=" * 80)
    print("STL Decomposition Module (Robustness Run)")
    print("=" * 80)
    ...
    setup_matplotlib()
    output_dir_path = create_output_directory()
    ...
    save_stl_results_csv(stl_results, output_dir_path, time_index)
    
    series_names = list(stl_results.keys())
    stl_3d_array, metadata = save_stl_arrays(stl_results, output_dir_path, time_index, series_names)
    
    generate_visualizations(stl_results, output_dir_path, time_index)
